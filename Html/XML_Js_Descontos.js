function loadData() {
    ...
    oXHR.open("GET", "library_Descontos.xml", true);        // true = ASYNCHRONOUS REQUEST (DESIRABLE), false = SYNCHRONOUS REQUEST.
    oXHR.send();
}
...
strongPreco.innerHTML=Produto.getElementsByTagName("precoAtual")[0].childNodes[0].nodeValue;
var strongPrecoAntigo=document.createElement('strong');
strongPrecoAntigo.className="preço-antigo";
strongPrecoAntigo.innerHTML=Produto.getElementsByTagName("precoAntigo")[0].childNodes[0].nodeValue;


divPreco.appendChild(strongPreco);
divPreco.appendChild(strongPrecoAntigo);
//--------------------------------
